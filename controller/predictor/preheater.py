...
from domain.types import FeedstockObservation, PreheaterCellState, PreheaterState, PreheaterDiagnostics, PreheaterOutput
...
class PreheaterForwardModel:
    ...
    def clone(self) -> "PreheaterForwardModel":
        """Return a fast structural clone for repeated NMPC rollouts.

        The previous implementation used ``copy.deepcopy``.  That is convenient,
        but SLSQP may clone the predictor hundreds of times per solve.  A manual
        clone copies only the mutable numeric state and the inlet-delay queue,
        while sharing immutable configuration objects.
        """
        other = PreheaterForwardModel(self.cfg)
        other.time_s = float(self.time_s)
        other.T_solid = self.T_solid.copy()
        other.omega0 = self.omega0.copy()
        other.tref = self.tref.copy()
        other.slope = self.slope.copy()
        other.bulk_density = self.bulk_density.copy()
        other.dry_mass = self.dry_mass.copy()
        other.water_mass = self.water_mass.copy()
        other.omega = self.omega.copy()
        other._last_Tg_profile = tuple(self._last_Tg_profile)
        other.last_diagnostics = self.last_diagnostics
        other._feed_delay = list(self._feed_delay)
        other._feed_delay_time_s = float(self._feed_delay_time_s)
        return other

    def load_state(self, state: PreheaterState, *, feedstock: FeedstockObservation | None = None) -> "PreheaterForwardModel":
        """Synchronize this predictor model from an estimated preheater state.

        The state may come from a Python plant snapshot, a COMSOL projection,
        or the controller estimator's own digital twin.  Feed delay history is
        reset to the current characterized feedstock when supplied, which keeps
        future rollout semantics well-defined even when the external plant does
        not expose its internal inlet-delay queue.
        """
        cells = list(state.cells)
        n = self.cfg.n_cells
        if len(cells) != n:
            if not cells:
                raise ValueError("cannot load an empty PreheaterState")
            # Resample by nearest normalized axial position if a backend uses a
            # different projection count.  Normal operation should remain n=20.
            src_n = len(cells)
            resampled = []
            for i in range(n):
                j = min(src_n - 1, max(0, int(round(i * (src_n - 1) / max(n - 1, 1)))))
                resampled.append(cells[j])
            cells = resampled

        self.time_s = float(state.time_s)
        self.omega0[:] = [float(c.omega0) for c in cells]
        self.tref[:] = [float(c.tref_min) for c in cells]
        self.slope[:] = [float(c.slope_min_per_c) for c in cells]
        self.T_solid[:] = [float(c.T_solid_C) for c in cells]
        self.dry_mass[:] = [max(float(c.dry_mass_kg), 1e-12) for c in cells]
        self.water_mass[:] = [max(float(c.water_mass_kg), 0.0) for c in cells]
        self.bulk_density[:] = [
            float(c.bulk_density_kg_m3) if c.bulk_density_kg_m3 is not None else self.opt_cfg.RHO_BULK
            for c in cells
        ]
        self._sync_omega_from_masses()
        self._last_Tg_profile = tuple(state.Tg_profile_C) if state.Tg_profile_C else tuple([175.0] * (n + 1))

        if feedstock is not None:
            eq = self._equivalent_from_feed(feedstock)
            t = float(feedstock.time_s)
            self._feed_delay = [
                (t - max(self.cfg.feed_delay_s, 0.0) - 1e-6, eq),
                (t, eq),
            ]
            self._feed_delay_time_s = 0.0
        return self
    ...
    def _apply_cell_energy(self, i: int, E_kJ: float, dt_s: float) -> tuple[float, float, float]:
        ...
        # Do not synchronize all cell moisture values here.  This method is
        # called once per cell; synchronizing the whole vector per cell turns a
        # single preheater step into O(n_cells^2) array work.  The caller syncs
        # once after all cell energy updates have been applied.
    ...
    def step_output(self, feed: FeedstockObservation, Tg_in_C: float, vg_mps: float, dt_s: float) -> PreheaterOutput:
        """Advance the predictor and return only the outlet quantities.

        NMPC objective evaluation only needs outlet moisture/temperature and mass
        flow diagnostics.  Returning a full 20-cell ``PreheaterState`` at every
        rollout step allocates hundreds of thousands of dataclass objects per
        full SLSQP solve.  This lightweight path preserves the same state update
        equations but avoids building the full cell snapshot.
        """
        ...
        return self.output(time_s=feed.time_s)

    def step_fast(self, feed: FeedstockObservation, Tg_in_C: float, vg_mps: float, dt_s: float) -> PreheaterOutput:
        """Compatibility alias for the lightweight NMPC rollout path."""
        return self.step_output(feed, Tg_in_C, vg_mps, dt_s)

    def step(self, feed: FeedstockObservation, Tg_in_C: float, vg_mps: float, dt_s: float) -> PreheaterState:
        self.step_output(feed, Tg_in_C, vg_mps, dt_s)
        return self.state(time_s=feed.time_s)

    def output(self, *, time_s: float | None = None) -> PreheaterOutput:
        self._sync_omega_from_masses()
        diag = self.last_diagnostics
        return PreheaterOutput(
            time_s=float(self.time_s if time_s is None else time_s),
            omega_out=float(self.omega[-1]),
            T_solid_out_C=float(self.T_solid[-1]),
            wet_out_kgps=float(diag.wet_out_kgps),
            dry_out_kgps=float(diag.dry_out_kgps),
            water_out_kgps=float(diag.water_out_kgps),
            Tg_out_C=float(diag.Tg_out_C),
        )
